    ...
    <section className="portal-hero relative min-h-[610px] overflow-hidden bg-brand-navy text-primary-foreground">
      ...
      <div className="relative mx-auto flex min-h-[610px] max-w-6xl items-center px-4 py-20 md:px-6">
        <div className="flex max-w-3xl flex-col items-start gap-6">
          <Badge className="rounded-full bg-brand-pink text-primary-foreground">Your gateway to international life in Lingshui</Badge>
          <h1 className="text-balance text-5xl font-black leading-tight tracking-tight text-primary-foreground md:text-7xl">New to Lingshui?<br />Feel at home.</h1>
          <p className="max-w-2xl text-pretty text-lg leading-relaxed text-primary-foreground/85">Practical guides, trusted resources, and community connections for international teachers, students, professionals, and families living in Lingshui.</p>
          <form action="/search" className="portal-search flex w-full items-center gap-2 bg-card text-card-foreground">
            <Search className="ml-2 size-5 text-muted-foreground" aria-hidden="true" /><label className="sr-only" htmlFor="hero-search">Search resources</label>
            <input id="hero-search" name="q" className="min-w-0 flex-1 bg-transparent px-2 py-2 outline-none" placeholder="Search visas, healthcare, housing, schools, or events" />
            <Button type="submit">Search</Button>
          </form>
          <div className="flex flex-wrap items-center gap-3 text-sm"><span className="text-primary-foreground/70">Popular:</span>{['Visa & Residency','Healthcare','Housing','Work & Onboarding'].map(item => <Link key={item} href={`/search?q=${encodeURIComponent(item)}`} className="border-b border-primary-foreground/50 hover:border-primary-foreground">{item}</Link>)}</div>
        </div>
      </div>
    </section>

    <section className="portal-intro border-b"><div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-8 md:flex-row md:items-start md:gap-16 md:px-6"><h2 className="shrink-0 font-serif text-2xl font-semibold">Welcome to Lingshui.</h2><p className="max-w-3xl text-pretty leading-relaxed text-muted-foreground">Built around the International Education Innovation Pilot Zone and everyday life in Lingshui, this platform connects essential information, service resources, and community activities. Professional matters are supported through clear guidance and referrals to qualified providers.</p></div></section>

    <section id="resources" className="mx-auto max-w-6xl px-4 py-20 md:px-6">
      <SectionHead kicker="Everyday essentials, clearly explained" title="Essential Resources in Lingshui" desc="Start with clear, practical information, then connect with the appropriate authority, organization, or service provider." />
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{resources.map(({ icon: Icon, title, desc }) => <Card key={title} className="portal-resource group"><CardHeader><span className="flex size-11 items-center justify-center rounded-xl bg-gradient-to-br from-brand-violet to-primary text-primary-foreground shadow-md"><Icon className="size-5" aria-hidden="true" /></span><CardTitle>{title}</CardTitle><CardDescription className="leading-relaxed">{desc}</CardDescription></CardHeader><CardContent><Link href={`/search?q=${encodeURIComponent(title)}`} className="inline-flex items-center gap-2 text-sm font-semibold text-primary">View resources <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></Link></CardContent></Card>)}</div>
    </section>

    <section id="guides" className="bg-muted"><div className="mx-auto max-w-6xl px-4 py-20 md:px-6">
      <SectionHead kicker="The expat guide to Lingshui" title="A Smoother Start in Lingshui" desc="Straightforward, practical answers to common questions about working, studying, and living here." />
      <div className="grid gap-6 lg:grid-cols-[1.4fr_.6fr]"><div className="grid gap-5 sm:grid-cols-2">{guides.map((guide, i) => <Link href={`/guides/${i + 1}`} key={guide.title} className="portal-guide group overflow-hidden border bg-card"><div className="relative aspect-[4/3] overflow-hidden"><Image src={guide.image} alt={guide.alt} fill className="object-cover transition-transform duration-500 group-hover:scale-105" sizes="(min-width: 1024px) 32vw, (min-width: 640px) 50vw, 100vw" /><div className="absolute inset-0 bg-gradient-to-t from-foreground/35 to-transparent" /><Badge variant="secondary" className="absolute bottom-4 left-4">{guide.tag}</Badge></div><div className="p-6"><h3 className="text-balance text-xl font-semibold group-hover:text-primary">{guide.title}</h3><p className="mt-3 text-sm leading-relaxed text-muted-foreground">{guide.desc}</p><span className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-primary">Read guide <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></span></div></Link>)}</div><aside className="portal-help flex min-h-96 flex-col justify-between p-7 text-primary-foreground"><div className="flex flex-col gap-4"><MapPin className="size-7" /><p className="text-sm font-semibold text-primary-foreground/70">New to Lingshui?</p><h3 className="font-serif text-3xl font-semibold">Not sure where to start?</h3><p className="leading-relaxed text-primary-foreground/75">Tell us what you need, and we&apos;ll point you to the right guide, responsible authority, or verified service resource.</p></div><Button render={<Link href="/request" />} nativeButton={false} variant="secondary">Get guidance <ArrowRight data-icon="inline-end" /></Button></aside></div>
    </div></section>

    <section id="services" className="mx-auto max-w-6xl px-4 py-20 md:px-6"><SectionHead kicker="How the platform helps" title="From Questions to the Right Resources" desc="Clear information, transparent referrals, and ongoing follow-up for more complex needs." /><div className="grid gap-8 md:grid-cols-3">{[['Find clear information','Browse practical guides, common questions, and useful checklists.'],['Match the right resources','Explore healthcare, housing, education, and professional services by category.'],['Submit and track a request','Record a complex issue and follow its status in your account.']].map(([title,desc],i)=><article key={title} className="flex flex-col gap-4 border-t-2 border-primary pt-6"><span className="font-serif text-4xl text-primary/45">0{i+1}</span><h3 className="text-xl font-semibold">{title}</h3><p className="leading-relaxed text-muted-foreground">{desc}</p></article>)}</div></section>

    <section id="events"><div className="mx-auto max-w-6xl px-4 py-20 md:px-6"><SectionHead kicker="Events & updates" title="International Connections, Local Experiences" desc="Make your new home feel like a community where you belong." /><div className="grid gap-6 lg:grid-cols-[1.1fr_.9fr]"><div className="portal-event-main relative overflow-hidden"><Image src="/images/community-gathering.png" alt="International community gathering in Lingshui" fill className="object-cover" sizes="(min-width: 1024px) 55vw, 100vw" /><div className="absolute inset-0 bg-gradient-to-t from-brand-navy via-brand-violet/25 to-brand-pink/10" /><div className="absolute inset-x-0 bottom-0 p-7"><Badge variant="secondary">Featured community event</Badge><h3 className="mt-4 font-serif text-3xl font-semibold">International Education & Community Meetup</h3><p className="mt-2 text-primary-foreground/75">Connecting educators, students, entrepreneurs, and local partners.</p></div></div><div className="flex flex-col divide-y overflow-hidden rounded-2xl border bg-card">{events.map(([day,month,title])=><article key={title} className="flex items-center gap-5 py-5 first:pt-0"><div className="flex size-16 shrink-0 flex-col items-center justify-center text-brand-pink"><strong className="text-xl">{day}</strong><small>{month}</small></div><div><h3 className="font-semibold">{title}</h3><p className="mt-1 text-sm text-muted-foreground">Open to all · Free registration</p></div></article>)}</div></div></div></section>

    <section id="community" className="portal-community px-4 py-20 md:px-6"><div className="mx-auto max-w-6xl"><SectionHead kicker="Community conversations" title="Ask, Share, Belong" desc="Real questions and shared experience make local knowledge more useful." inverse /><div className="grid gap-6 lg:grid-cols-[1.3fr_.7fr]"><div className="flex flex-col divide-y rounded-2xl border bg-card px-6">{['What should I arrange first after arriving in Lingshui?','Where can I join language practice and cultural activities?','What should international residents know about local healthcare?'].map((title,i)=><article key={title} className="py-6"><div className="flex items-start gap-4"><MessageCircle className="mt-1 size-5 shrink-0 text-primary" /><div><h3 className="font-semibold">{title}</h3><p className="mt-2 text-sm text-muted-foreground">Community Q&A · {5+i*3} replies · Recently updated</p></div></div></article>)}</div><aside className="flex flex-col justify-between rounded-3xl bg-[linear-gradient(145deg,rgb(113,57,213),rgb(249,47,157)_56%,rgb(255,119,95))] p-7 text-brand-navy shadow-xl shadow-brand-pink/20"><div><Languages className="size-7" /><h3 className="mt-6 font-serif text-3xl font-semibold">Build your local connections here.</h3><p className="mt-4 leading-relaxed opacity-75">Ask a question, join an event, or find your way to community resources.</p></div><Button render={<Link href="/request" />} nativeButton={false}>Join the community</Button></aside></div></div></section>
  </div>
}
