...
export function PortalHome() {
  return <>
    <section className="relative min-h-[610px] overflow-hidden bg-primary text-primary-foreground">
      <Image src="/images/lingshui-coast.png" alt="陵水热带海岸与国际社区" fill priority className="object-cover opacity-55" sizes="100vw" />
      ...
      <div className="relative mx-auto flex min-h-[610px] max-w-6xl items-center px-4 py-20 md:px-6">
        <div className="flex max-w-3xl flex-col items-start gap-6">
          <Badge variant="secondary">你的陵水国际生活服务入口</Badge>
          <h1 className="text-balance font-serif text-5xl font-semibold leading-tight md:text-7xl">初到陵水，<br />安心生活。</h1>
    ...
    <section id="resources" className="mx-auto max-w-6xl px-4 py-20 md:px-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{resources.map(({ icon: Icon, title, desc }) => <Card key={title} className="group transition-transform hover:-translate-y-1"><CardHeader><span className="flex size-11 items-center justify-center rounded-xl bg-secondary text-primary"><Icon className="size-5" aria-hidden="true" /></span><CardTitle>{title}</CardTitle><CardDescription className="leading-relaxed">{desc}</CardDescription></CardHeader><CardContent><Link href={`/search?q=${encodeURIComponent(title)}`} className="inline-flex items-center gap-2 text-sm font-semibold text-primary">查看资源 <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></Link></CardContent></Card>)}</div>
    </section>
