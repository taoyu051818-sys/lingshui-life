...
export function PortalHome() {
  return <>
    <section className="relative min-h-[610px] overflow-hidden bg-brand-navy text-primary-foreground">
      ...
      <div className="relative mx-auto flex min-h-[610px] max-w-6xl items-center px-4 py-20 md:px-6">
        <div className="flex max-w-3xl flex-col items-start gap-6">
          ...
          <h1 className="text-balance text-5xl font-black leading-tight tracking-tight md:text-7xl">初到陵水，<br /><span className="bg-gradient-to-r from-brand-gold via-accent to-brand-electric bg-clip-text text-transparent">安心生活。</span></h1>
          ...
          <form action="/search" className="flex w-full max-w-2xl items-center gap-2 rounded-xl bg-card p-2 text-card-foreground shadow-xl">
    ...
    <section id="resources" className="mx-auto max-w-6xl px-4 py-20 md:px-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{resources.map(({ icon: Icon, title, desc }) => <Card key={title} className="group border-t-4 border-t-brand-pink shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-brand-violet/10"><CardHeader><span className="flex size-11 items-center justify-center rounded-xl bg-gradient-to-br from-brand-violet to-primary text-primary-foreground shadow-md"><Icon className="size-5" aria-hidden="true" /></span><CardTitle>{title}</CardTitle><CardDescription className="leading-relaxed">{desc}</CardDescription></CardHeader><CardContent><Link href={`/search?q=${encodeURIComponent(title)}`} className="inline-flex items-center gap-2 text-sm font-semibold text-primary">查看资源 <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></Link></CardContent></Card>)}</div>
    </section>
    ...
    <section id="events" className="bg-brand-navy text-primary-foreground"><div className="mx-auto max-w-6xl px-4 py-20 md:px-6"><SectionHead kicker="活动与资讯" title="国际交流与本地体验" desc="让新的居住地真正成为有归属感的社区。" inverse /><div className="grid gap-6 lg:grid-cols-[1.1fr_.9fr]"><div className="relative min-h-80 overflow-hidden rounded-2xl"><Image src="/images/community-gathering.png" alt="陵水国际社区交流活动" fill className="object-cover" sizes="(min-width: 1024px) 55vw, 100vw" /><div className="absolute inset-0 bg-gradient-to-t from-brand-navy via-brand-violet/25 to-brand-pink/10" /><div className="absolute inset-x-0 bottom-0 p-7"><Badge variant="secondary">社区特色活动</Badge><h3 className="mt-4 font-serif text-3xl font-semibold">国际教育与社区交流会</h3><p className="mt-2 text-primary-foreground/75">连接教育工作者、学生、创业者与本地伙伴。</p></div></div><div className="flex flex-col divide-y divide-primary-foreground/20">{events.map(([day,month,title])=><article key={title} className="flex items-center gap-5 py-5 first:pt-0"><div className="flex size-16 shrink-0 flex-col items-center justify-center rounded-xl bg-primary-foreground text-primary"><strong className="text-xl">{day}</strong><small>{month}</small></div><div><h3 className="font-semibold">{title}</h3><p className="mt-1 text-sm text-primary-foreground/65">开放交流 · 免费预约</p></div></article>)}</div></div></div></section>

    <section id="community" className="mx-auto max-w-6xl px-4 py-20 md:px-6"><SectionHead kicker="社区话题交流" title="提问、分享、融入" desc="真实的问题与经验交流，让本地信息发挥更大价值。" /><div className="grid gap-6 lg:grid-cols-[1.3fr_.7fr]"><div className="flex flex-col divide-y rounded-2xl border bg-card px-6">{['刚到陵水，优先需要办理哪些生活事项？','哪里可以参加中文练习和文化体验活动？','国际友好型医疗就诊有哪些实用建议？'].map((title,i)=><article key={title} className="py-6"><div className="flex items-start gap-4"><MessageCircle className="mt-1 size-5 shrink-0 text-primary" /><div><h3 className="font-semibold">{title}</h3><p className="mt-2 text-sm text-muted-foreground">社区问答 · {5+i*3} 条回复 · 最近更新</p></div></div></article>)}</div><aside className="flex flex-col justify-between rounded-3xl bg-gradient-to-br from-brand-gold to-accent p-7 text-brand-navy shadow-xl shadow-brand-gold/20"><div><Languages className="size-7" /><h3 className="mt-6 font-serif text-3xl font-semibold">从这里建立本地连接。</h3><p className="mt-4 leading-relaxed opacity-75">提出问题、参与活动或获取社区资源指引。</p></div><Button render={<Link href="/request" />} nativeButton={false}>加入社区交流</Button></aside></div></section>
  </>
}
